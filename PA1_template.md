...
This step involves loading required libraries and setting the working directory, so our code will
function properly:
...
In this assignment, we use the `data.table` package, which provides high performance functions to 
load files and deal with tables. Since the data file is zipped, we first unzip it, then read the
...
#### 1) Make a time series plot f the 5-minute interval (x-axis) and the average number of steps taken, averaged across all days (y-axis)

First, we must calculate the average steps for each 5-minute interval, across all days in the dataset.
Note that we create a new column in the calculation data table with the time interval padded with
leading zeros. Therefore, strings like `105` become `0105`. The reason for this transformation
...
In order to present the x-axis scale in the plot in a more meaningful way (i.e. in the `%H:%M` format),
we calculate an auxiliary character that takes the padded time interval strings and gererates
Date/Time objects from them (note the date is not important. We are only interested in the time):
...
Finally, we can then plot the data series, using the `timeVector` as labels for the x-axis:
...
particular 5-minute interval. One efficient way of doing this is merging the `data` and the
`avgFiveMinuteInt` data tables to create one bigger data table with separate columns for the actual
...
In order to differentiate from the preivou calculated values, we'll add a suffix to the values
calculated in this step. The calculation is otherwise the same as done before, but using the
...
As we can see, the values do differ from the preivously calculated values. The immediate effect of 
...
Finally, we create the panel plot, using the Date/Time vector as labels for the x-axis:
